...
pub struct TurnConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub soft_timeout_config: Option<SoftTimeoutConfig>,
}

impl TurnConfig {
    pub fn builder() -> TurnConfigBuilder {
        <TurnConfigBuilder as Default>::default()
    }
}
...
pub struct TurnConfigBuilder {
    ...
    soft_timeout_config: Option<SoftTimeoutConfig>,
}
...
impl TurnConfigBuilder {
    ...
    pub fn soft_timeout_config(mut self, value: SoftTimeoutConfig) -> Self {
        self.soft_timeout_config = Some(value);
        self
    }

    /// Consumes the builder and constructs a [`TurnConfig`].
    pub fn build(self) -> Result<TurnConfig, BuildError> {
        Ok(TurnConfig {
