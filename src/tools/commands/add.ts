...
export function createAddCommand(): Command {
  return new Command('add')
    ...
    .action(async (name: string, options: { type: string; configPath?: string; env: string }) => {
      try {
        ...
        await addTool(name, options.type as 'webhook' | 'client', options.configPath, options.env);
      } catch (error) {
