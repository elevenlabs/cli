...
export async function getElevenLabsClient(environment: string = 'prod'): Promise<ElevenLabsClient> {
  ...
  if (!apiKey) {
    throw new Error(`No API key found for environment '${environment}'. Use 'elevenlabs login --env ${environment}' to authenticate or set ELEVENLABS_API_KEY environment variable.`);
  }
